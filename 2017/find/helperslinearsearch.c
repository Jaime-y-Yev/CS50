...
{
    int c = 1; // counts swaps, initial value must be different from 0
    while (c != 0) // during first loop and while changes are recognized in subsequent loops
    {
        c = 0; // reset counter
        for (int i = 0; i < n; i++) // loop through adjacent positions
        {
